...
abstract class ModelService
{
    ...
    public function findByIdWithRelation(int $id, array $relation = [], array $count = [])
    {
        ...
        return $this->model->query()->with($relation)->withCount($count)->find($id);
    }

    public function allWithRelation(array $relation = [], array $count = [])
    {
        if (request()->pagination) {
            return $this->model->query()->with($relation)->withCount($count)->orderByDesc('created_at')->paginate(request()->pagination);
        } else {
            return $this->model->query()->with($relation)->withCount($count)->orderByDesc('created_at')->get();
        }
    }
